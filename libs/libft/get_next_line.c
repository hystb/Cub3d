...
/*   Updated: 2022/12/22 11:31:30 by nmilan           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*read_the_line(char *buffer, int *end_file, int fd)
{
	...
	while (buffer[i] != '\n' && *end_file > 0)
	{
		if (buffer[i++] == '\0' && *end_file > 0)
			line = ft_strjoin_gnl(line, next_line(buffer, end_file, &i, fd));
	}
	return (line);
}
...
void	clean_buf(char *buffer, int *end_file)
{
	...
	if (i + 1 == BUFFER_SIZE)
		...
		ft_memmove_gnl(buffer, buffer + i + 1, ft_strlen_gnl(buffer + i + 1));
}
